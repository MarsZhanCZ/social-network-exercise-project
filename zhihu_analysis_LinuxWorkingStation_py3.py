...
def density_centrality():
    ...
    user_betweenness_list = sorted(nx.betweenness_centrality(G).items(),key=len, reverse=True) #result like [(2, 0.0), (3, 0.0), (1, 1.0)]
    betweenness_list = list(zip(*user_betweenness_list))[1]# result like: [(2, 3, 1), (0.0, 0.0, 1.0)][1]; zip(*) is like un zip()
    ...
    user_closeness_list = sorted(nx.closeness_centrality(G).items(),key=len, reverse=True) #Dict.items(): {1: 1.0, 2: 0.0, 3: 0.0} -> [(1, 1.0), (2, 0.0), (3, 0.0)]
